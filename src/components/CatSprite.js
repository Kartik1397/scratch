...
export default function CatSprite({ onClick, transform, id, defautlActive = false }) {
  const sprite = useRef(null);

  useEffect(() => {
    ActionManager.sprites[id] = sprite.current;
    if (defautlActive) {
      ActionManager.Sprite.setActiveSprite(sprite.current);
    }
  });
